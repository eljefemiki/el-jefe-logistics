...
export default function FinanceStats({ stats }: { stats: { invoiced: number; collected: number; outstanding: number; overdue: number; overdueCount: number; journeyIncome: number; journeyCosts: number; journeyProfit: number } }) {
  const cards = [
    ...
    { label: "Journey income", value: money.format(stats.journeyIncome), icon: CirclePoundSterling, colour: "text-emerald-400" },
    { label: "Journey costs", value: money.format(stats.journeyCosts), icon: Banknote, colour: "text-amber-400" },
    { label: "Journey profit", value: money.format(stats.journeyProfit), icon: HandCoins, colour: stats.journeyProfit >= 0 ? "text-emerald-400" : "text-red-400" },
  ];
