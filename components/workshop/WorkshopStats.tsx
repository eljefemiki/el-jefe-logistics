...
export default function WorkshopStats({ stats }: { stats: { active: number; critical: number; waitingParts: number; scheduled: number; estimatedExposure: number } }) {
  const cards = [
  ...
  return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{cards.map(({ label, value, icon: Icon, colour }) => (
